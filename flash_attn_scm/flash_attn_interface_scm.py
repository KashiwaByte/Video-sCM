...
#         """
#         Define the forward-mode automatic differentiation formula for FlashAttnVarlenFunc.
        
#         Args:
#             ctx: Context object with saved tensors from forward()
#             *grad_inputs: Gradients w.r.t. each input of forward()
#                         (None for non-tensor inputs)
        
#         Returns:
#             Gradients w.r.t. each output of forward()
#         """
#         # Unpack the gradient inputs (tangents)
#         (dq, dk, dv, *_) = grad_inputs
...
#         return dq,dk
        ...
        # dout_q = torch.zeros_like(q) if dq is not None else None
        # dout_k = torch.zeros_like(k) if dk is not None else None
        # dout_v = torch.zeros_like(v) if dv is not None else None
        
        # # Compute JVP for each input that has a gradient
        # if dq is not None:
        #     # Compute contribution from dq
        #     temp_out, _, _, _, _, _, _, _ = _flash_attn_varlen_forward(
        #         dq,
        #         k,
        #         v,
        #         cu_seqlens_q,
        ...
        #         return_softmax=False,
        #         block_table=None,  # block_table is not used in forward pass for JVP
        #     )
        #     dout_q = temp_out
        
        # if dk is not None:
        #     # Compute contribution from dk
        #     temp_out, _, _, _, _, _, _, _ = _flash_attn_varlen_forward(
        #         q,
        #         dk,
        #         v,
        ...
        #         block_table=None,
        #     )
        #     if dout_q is not None:
        #         dout_q = dout_q + temp_out
        #     else:
        #         dout_q = temp_out
        
        # if dv is not None:
        #     # Compute contribution from dv
        #     temp_out, _, _, _, _, _, _, _ = _flash_attn_varlen_forward(
        #         q,
        #         k,
        #         dv,
        ...
        # # Handle the case where return_softmax was True in forward
        # if ctx.needs_input_grad[14]:  # return_softmax flag
        #     # For simplicity, we won't compute JVP for softmax_lse and S_dmask
        #     return (dout_q, None, None)
        # else:
        #     return (dout_q,)
