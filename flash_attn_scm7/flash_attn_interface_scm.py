...
def _flash_attn_varlen_forward(
    ...
    #     breakpoint()
    return out.clone(), softmax_lse.clone(), S_dmask.clone() if S_dmask is not None else None, rng_state.clone() if rng_state is not None else None
...
#         # ctx.save_for_forward(q, k, v, out_padded, softmax_lse, cu_seqlens_q, cu_seqlens_k, rng_state)
#         if is_grad:
#             ctx.save_for_backward(
#                 q, k, v, out_padded, softmax_lse, cu_seqlens_q, cu_seqlens_k, rng_state
#             )
...
#         # Compute forward pass with tangents
#         out_tangent, _, _, _ = _wrapped_flash_attn_varlen_forward(
#             dq,
#             k,
#             v,
#             cu_seqlens_q,
...
class FlashAttnVarlenFunc(torch.autograd.Function):
    ...
    def jvp(ctx, *tangents):
        return tangents[:2]
        # """
        # Corrected Jacobian-vector product implementation that matches 
        # the actual signature of _wrapped_flash_attn_varlen_forward
        # """
        # # 1. Retrieve saved tensors from forward pass
        # q, k, v, out, softmax_lse, cu_seqlens_q, cu_seqlens_k, S_dmask = ctx.saved_for_forward
        
        # # 2. Process input gradients and handle None cases explicitly
        # dq, dk, dv = tangents[:3]
        # # We still compute gradients even if some are None, as per tutorial requirements
        # compute_grad = not all(t is None for t in [dq, dk, dv])

        # # 3. Prepare tensors with view/inplace behavior matching forward
        # device = q.device
        # head_size_og = out.size(-1)
        # needs_padding = (head_size_og % 8 != 0)

        # def _prepare_input(t, base_tensor):
        #     if t is None:
        #         # Create a zero tensor with same shape as base for consistent behavior
        #         t = torch.zeros_like(base_ten our own storage
        #     t = t.clone()
        #     # Ensure contiguous memory layout
        #     t = t.contiguous()
        #     if needs_padding:
        #         t = F.pad(t, [0, 8 - head_size_og % 8])
        #     return t

        # # 4. Initialize output maintaining view relationships
        # out_tangent = torch.empty_like(out)
        # if needs_padding:
        #     out_tangent_padded = F.pad(out_tangent, [0, 8 - head_size_og % 8])
        # else:
        #     out_tangent_padded = out_tangent

        # # 5. Compute contributions with proper gradient handling
        # def _compute_contrib(grad, fixed1, fixed2, base1, base2):
        #     # Ensure all inputs have their own storage and are contiguous
        #     grad_prepared = _prepare_input(grad, base1)
        #     fixed1_prepared = _prepare_input(fixed1, base1)
        #     fixed2_prepared = _prepare_input(fixed2, base2)
            
        #     # Use cloned tensors to avoid storage issues
        #     cu_seqlens_q_cont = cu_seqlens_q.clone().contiguous()
        #     cu_seqlens_k_cont = cu_seqlens_k.clone().contiguous()
            
        #     contrib = _wrapped_flash_attn_varlen_forward(
        #         grad_prepared if grad is not None else fixed1_prepared,
        #         grad_prepared if grad is not None else fixed1_prepared,
        #         grad_prepared if grad is not None else fixed2_prepared,
        #         cu_seqlens_q_cont,
        #         cu_seqlens_k_cont,
        #         ctx.max_seqlen_q,
        #         ctx.max_seqlen_k,
        ...
        #         block_table=None  # Maintain consistency with forward
        #     )[0]
        #     # Clone before adding to ensure we have our own storage
        #     contrib = contrib.clone()
        #     # Inplace update to maintain view relationships
        #     out_tangent_padded.add_(contrib)

        # if compute_grad:
        #     _compute_contrib(dq, k, v, q, v)sor, device=device)
        #     # Clone to ensure we have
        #     _compute_contrib(dk, q, v, k, v)
        #     _compute_contrib(dv, q, k, v, k)

        # # 6. Remove padding if needed while preserving view relationships
        # if needs_padding:
        #     out_tangent.copy_(out_tangent_padded[..., :head_size_og])

        # # Clean up context data that's no longer needed

        # # Return gradients for all outputs to match forward's signature
        # return (out_tangent,) + (None,) * (len(tangents) - 1)
