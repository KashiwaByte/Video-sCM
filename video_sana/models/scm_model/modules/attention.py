...
try:
    ...
    warnings.warn('xformers is not available, falling back to manual attention implementation')
...
def manual_varlen_attention(
    ...
    if XFORMERS_AVAILABLE:
        ...
        return output.unflatten(0, (batch_size, max_seqlen_q))
    else:
        # 如果xformers不可用，回退到原始实现
        batch_size = cu_seqlens_q.size(0) - 1
        num_heads = q.size(1)
        head_dim = q.size(2)
...
# def _jvp_aware_attention(q, k, v, softmax_scale, causal):
#     """与torch.func.jvp兼容的注意力计算核心"""
#     # 使用torch.func兼容的操作
#     attn_scores = fc.vmap(lambda q, k: torch.einsum('hd,kd->k', q, k))(q, k.unsqueeze(0))
    
#     if softmax_scale is not None:
#         attn_scores = attn_scores * softmax_scale
    
#     if causal:
#         seq_len_q, seq_len_k = q.size(0), k.size(0)
#         mask = torch.triu(
...
#     attn_weights = torch.softmax(attn_scores, dim=-1)
#     out = torch.einsum('k,khd->hd', attn_weights, v)
#     return out

# def janual_varlen_attention(
#     q, k, v,
#     cu_seqlens_q, cu_seqlens_k,
#     max_seqlen_q, max_seqlen_k,
#     softmax_scale=None,
...
#         curr_q = q[q_start:q_end]
#         curr_k = k[k_start:k_end]
#         curr_v = v[k_start:k_end]
        
#         if torch.is_grad_enabled() and not deterministic:
#             # 使用检查点但保持JVP兼容性
#             out = checkpoint(
#                 _jvp_aware_attention,
...
#             )
#         else:
#             out = _jvp_aware_attention(curr_q, curr_k, curr_v, softmax_scale, causal)
